...
async function advancedMakeMoneyFromServer(ns, server) {
	...
	await actionServer(ns, server, growScript, calcGrowTime, calcGrowThreads)
	...
	await actionServer(ns, server, hackScript, calcHackTime, calcHackThreads)
}

/** @param {NS} ns */
async function actionServer(ns, targetServerHostname, script, calcActionTime, calcActionThreads) {
	const hostname = ns.getServer().hostname;
