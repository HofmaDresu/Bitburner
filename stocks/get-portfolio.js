...
export async function main(ns) {
	Object.keys(stockToServers)
		...
		.forEach(info => {
			if (info.value !== 0) {
				ns.tprint(`${info.stockSymbol}: value -> ${info.value.toLocaleString('en-US')}, position -> ${info.longShares > 0 ? "L" : ""}${info.shortShares > 0 ? "S" : ""}, profit -> ${(info.value - info.cost).toLocaleString('en-US')}, pct -> ${((info.value - info.cost) / info.cost * 100).toLocaleString('en-US')}%`);
			};
		});
}
